...
const helperProcessEnv = "GO_WANT_PAYMENT_CONFIG_HELPER_PROCESS"
...
func TestMustLoad(t *testing.T) {
	t.Run("loads embedded common config from env", func(t *testing.T) {
		...
		require.Equal(t, "payment-svc", cfg.Service.Name)
		require.Equal(t, ":18080", cfg.Service.HTTPAddr)
		require.Equal(t, "postgres", cfg.Postgres.Host)
		require.Equal(t, "payment", cfg.Postgres.Database)
		require.Equal(t, "payment", cfg.Postgres.User)
...
func setRequiredEnv(t *testing.T) {
	...
	t.Setenv("SERVICE_NAME", "payment-svc")
	t.Setenv("POSTGRES_HOST", "postgres")
	t.Setenv("POSTGRES_DB", "payment")
