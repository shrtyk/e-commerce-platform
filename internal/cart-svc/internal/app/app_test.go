...
func TestApplicationRunValidation(t *testing.T) {
	tests := []struct {
		name    string
		app     *cartapp.Application
		errWant error
	}{
		{
			name:    "missing config",
			app:     cartapp.NewApplication(nil, nil, grpcpkg.NewServer(), nil, nil),
			errWant: cartapp.ErrConfigRequired,
		},
		{
			name: "missing grpc",
			app: cartapp.NewApplication(
				&config.Config{Config: commoncfg.Config{Service: commoncfg.Service{Name: "cart"}}},
				nil,
				nil,
				nil,
				nil,
			),
			errWant: cartapp.ErrGRPCServerRequired,
		},
	}
