...
func TestApplicationRunValidation(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "missing config",
			app:     orderapp.NewApplication(nil, nil, grpcpkg.NewServer(), nil),
			errWant: orderapp.ErrConfigRequired,
		},
		{
			name: "missing grpc",
			app: orderapp.NewApplication(
				&config.Config{Config: commoncfg.Config{Service: commoncfg.Service{Name: "order"}}},
				nil,
				nil,
				nil,
			),
			errWant: orderapp.ErrGRPCServerRequired,
		},
	}
